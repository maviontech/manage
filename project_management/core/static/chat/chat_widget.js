...
//     startUnreadPolling();
//     // focus search for quicker keyboard use
//     const s = document.getElementById('chat-member-search'); if (s) s.focus();
//   }
//   function hidePanel() {
...
//     const ob = document.getElementById('chat-open-btn'); if (ob) ob.focus();
//   }
...
//         li.dataset.id = memberId; li.dataset.email = memberId; li.dataset.name = memberName;
//         if (m.is_self) li.dataset.self = 'true';

//         // avatar (initials) + presence
//         const avatar = document.createElement('span');
//         avatar.className = 'member-avatar';
//         const initials = (memberName || '').split(' ').map(s=>s[0]).slice(0,2).join('').toUpperCase() || memberId.slice(0,2).toUpperCase();
//         avatar.textContent = initials;

//         const dot = document.createElement('span'); dot.className = 'presence-dot offline'; dot.setAttribute('data-user', memberId);
//         if (m.is_self) { dot.classList.remove('offline'); dot.classList.add('online'); }

//         const meta = document.createElement('div'); meta.className = 'member-meta';
//         const nameEl = document.createElement('span'); nameEl.className = 'member-name'; nameEl.textContent = memberName;
//         const subEl = document.createElement('span'); subEl.className = 'member-sub'; subEl.textContent = memberId;
//         meta.appendChild(nameEl); meta.appendChild(subEl);

//         const badge = document.createElement('span'); badge.className = 'member-badge'; badge.style.display = 'none'; badge.setAttribute('data-unread-for', memberId);
...
//       const q = normId(e.target.value || '');
//       [...membersList.children].forEach(li=>{
//         const name = normId(li.dataset.name || li.dataset.email || '');
...
//           total += (parseInt(u.count,10) || 0);
//         }
//       });
//       // update FAB unread total
//       const fabBadge = document.getElementById('chat-unread-badge');
//       if (fabBadge) {
//         if (total > 0) { fabBadge.style.display = 'inline-block'; fabBadge.textContent = total > 99 ? '99+' : String(total); }
...
//   // keyboard: Esc closes panel when open
