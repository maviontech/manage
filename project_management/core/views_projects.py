        ...
        if form.is_valid():
            ...
            try:
                cur.execute("""
                    ...
                    VALUES (%s,%s,%s,%s,%s,%s)
                """, (data['name'], data['description'], data['start_date'] or None, data['end_date'] or None,
...
def subprojects_list(request, project_id):
    ...
    try:
        ...
        cur.close(); conn.close()
    return render(request, "core/subprojects_list.html", {"project": project, "subprojects": subs})
...
def subproject_create(request, project_id):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            conn, cur = get_tenant_conn_and_cursor(request)
            try:
                cur.execute("INSERT INTO subprojects (project_id, name, description) VALUES (%s,%s,%s)",
                            (project_id, d['name'], d['description']))
                sub_id = cur.lastrowid
                cur.execute("INSERT INTO activity_log (entity_type, entity_id, action, performed_by) VALUES (%s,%s,%s,%s)",
                            ("subproject", sub_id, "created subproject", request.session.get('user_id')))
            finally:
                cur.close(); conn.close()
        ...
        form = SubprojectForm()
    return render(request, "core/subproject_create.html", {"form": form, "project_id": project_id})
...
def subproject_edit(request, project_id, sub_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            conn, cur = get_tenant_conn_and_cursor(request)
            try:
                cur.execute("UPDATE subprojects SET name=%s, description=%s WHERE id=%s", (d['name'], d['description'], sub_id))
                cur.execute("INSERT INTO activity_log (entity_type, entity_id, action, performed_by) VALUES (%s,%s,%s,%s)",
        ...
        form = SubprojectForm(initial={'name': s['name'], 'description': s['description']})
    return render(request, "core/subproject_create.html", {"form": form, "editing": True, "project_id": project_id, "sub": s})
