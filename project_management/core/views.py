...
def dashboard_view(request, personal=False):
    """
    Dashboard for logged-in user (tenant-aware).
    Computes top metrics and chart data for the logged-in user.

    personal=True renders the same unified dashboard scoped to only the current
    member (used by /user-dashboard/ "My View", the default for non-admins).
    """
    ...
    # My View (personal): scope every metric to just the current member.
    if personal:
        visible_user_ids = [member_id]
...
def user_dashboard_view(request):
    """My View — the unified dashboard scoped to only the current member.
    Delegates to dashboard_view(personal=True) so both views stay in sync
    (KPIs, time range, and all sections behave identically, just personal-scoped)."""
    return dashboard_view(request, personal=True)
